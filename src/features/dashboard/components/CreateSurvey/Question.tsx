...
export function Question({ sectionIndex, questionIndex, removeQuestion }: QuestionProps) {
  ...
  const currentQuestionType = watch(`sections[${sectionIndex}].questions[${questionIndex}].type`);
  const questionText = watch(`sections[${sectionIndex}].questions[${questionIndex}].text`);
        ...
        <>
          ...
          {errors?.sections?.[sectionIndex]?.questions?.[questionIndex]?.text && (
            <p className="text-sm text-red-600 mt-2">
              {errors.sections[sectionIndex].questions[questionIndex].text?.message}
            </p>
          )}
        </>
      )}
