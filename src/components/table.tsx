        ...
        <table className="w-full text-left border-collapse whitespace-nowrap">
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-slate-800">
            ...
            {bodyData.map((row, rowIndex) => (
              <tr key={rowIndex} className="hover:bg-gray-50 dark:hover:bg-slate-900 transition-colors">
                ...
                {Object.entries(row)
                  .filter(([key]) => key !== "id")
                  .map(([key, value]) => (
                    <td key={key} className="px-6 py-4 text-sm text-gray-600 dark:text-slate-300">
                      {value}
                    </td>
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600">
                    <div className="relative inline-block">
                      <button onClick={() => {handleAction(row); setActions(true)}} className="text-gray-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white cursor-pointer font-medium">
                      ...
                      {actions && activeRow === row && (
                        <>
                          ...
                          <div className="absolute right-0 top-full mt-1 z-50 w-40 rounded-lg border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-lg py-1">
                            <button onClick={() => onView && onView(row)} className="flex w-full items-center gap-3 px-4 py-2.5 text-sm text-gray-700 dark:text-slate-200 hover:bg-gray-50 dark:hover:bg-slate-900 transition-colors cursor-pointer">
                              {/* View icon */}
                              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                              View
                            </button>
                            <button onClick={() => onEdit && onEdit(row)} className="flex w-full items-center gap-3 px-4 py-2.5 text-sm text-gray-700 dark:text-slate-200 hover:bg-gray-50 dark:hover:bg-slate-900 transition-colors cursor-pointer">
                              {/* Edit icon */}
